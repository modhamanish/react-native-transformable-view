...
export const DraggableView = ({ id, initialX = 50, initialY = 50, initialWidth = 100, initialHeight = 100, containerWidth = 300, containerHeight = 300, children, onDelete, onCopy, isSelected, onSelect, onDeselect, }) => {
    // Position and size
    ...
    const dragGesture = Gesture.Pan()
        ...
        const constrainedCx = Math.max(-minRelX, Math.min(containerWidth - maxRelX, cx));
        const constrainedCy = Math.max(-minRelY, Math.min(containerHeight - maxRelY, cy));
    ...
    const clampToBoundaries = (w, h, r, tx, ty) => {
        "worklet";
        const cx = tx + w / 2;
        const cy = ty + h / 2;
        const w2 = w / 2;
    ...
    const resizeGesture = Gesture.Pan()
        ...
        const constrained = clampToBoundaries(newWidth, newHeight, rotation.value, targetX, targetY);
        // Check if size fits in container
        if (constrained.bboxW > containerWidth ||
            constrained.bboxH > containerHeight) {
            return;
        }
        // Check if position was shifted (hit a wall)
        if (Math.abs(constrained.x - targetX) > 1 ||
    ...
    const resizeHeightGesture = Gesture.Pan()
        ...
        const targetX = startX.value - shiftX;
        const targetY = startY.value - shiftY;
        const constrained = clampToBoundaries(width.value, newHeight, rotation.value, targetX, targetY);
        // Check if size fits in container
        if (constrained.bboxW > containerWidth ||
            constrained.bboxH > containerHeight) {
            return;
    ...
    const resizeWidthGesture = Gesture.Pan()
        ...
        const targetX = startX.value - shiftX;
        const targetY = startY.value - shiftY;
        const constrained = clampToBoundaries(newWidth, height.value, rotation.value, targetX, targetY);
        // Check if size fits in container
        if (constrained.bboxW > containerWidth ||
            constrained.bboxH > containerHeight) {
            return;
    ...
    return (_jsxs(_Fragment, { children: [_jsx(GestureDetector, { gesture: mainGesture, children: _jsx(Animated.View, { style: [
                        styles.draggableView,
                        animatedViewStyle,
                        isSelected && styles.selected,
                    ], children: children || _jsx(View, { style: styles.defaultContent }) }) }), isSelected && (_jsxs(_Fragment, { children: [_jsx(GestureDetector, { gesture: Gesture.Tap().onEnd(() => {
                            runOnJS(onDeselect)();
                        }), children: _jsx(Animated.View, { style: [StyleSheet.absoluteFill, { zIndex: -1 }] }) }), _jsxs(Animated.View, { style: [styles.buttonContainer, animatedButtonContainerStyle], pointerEvents: "box-none", children: [_jsx(GestureDetector, { gesture: deleteGesture, children: _jsx(Animated.View, { style: [
                                        styles.actionButton,
                                        styles.topLeft,
                                        counterRotationStyle,
                                    ], children: _jsx(Text, { style: styles.emoji, children: "\u274C" }) }) }), _jsx(GestureDetector, { gesture: copyGesture, children: _jsx(Animated.View, { style: [
                                        styles.actionButton,
                                        styles.topRight,
                                        counterRotationStyle,
                                    ], children: _jsx(Text, { style: styles.emoji, children: "\uD83D\uDCCB" }) }) }), _jsx(GestureDetector, { gesture: resizeHeightGesture, children: _jsx(Animated.View, { style: [styles.actionButton, styles.bottomCenter], children: _jsx(Text, { style: styles.emoji, children: "\u2195\uFE0F" }) }) }), _jsx(GestureDetector, { gesture: resizeWidthGesture, children: _jsx(Animated.View, { style: [styles.actionButton, styles.rightCenter], children: _jsx(Text, { style: styles.emoji, children: "\u2194\uFE0F" }) }) }), _jsx(GestureDetector, { gesture: resizeGesture, children: _jsx(Animated.View, { style: [styles.actionButton, styles.bottomRight], children: _jsx(Text, { style: styles.emoji, children: "\u2198\uFE0F" }) }) }), _jsx(GestureDetector, { gesture: rotateGesture, children: _jsx(Animated.View, { style: [
                                        styles.actionButton,
                                        styles.bottomLeft,
                                        counterRotationStyle,
                                    ], children: _jsx(Text, { style: styles.emoji, children: "\uD83D\uDD04" }) }) })] })] }))] }));
};
